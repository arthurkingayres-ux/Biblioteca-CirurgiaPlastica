...
- `webapp/library/sw.js` (the service worker — project convention uses `sw.js`, not `service-worker.js`)
...
- `webapp/library/sw.js` holds a `CACHE_NAME` constant (current convention: `briefing-preop-v<N>`). When any asset under `webapp/library/` changes, the version must bump **in the same diff** — otherwise clients keep the stale cache.
...
1. `Read webapp/library/sw.js` — note the current `CACHE_NAME` value.
2. `Grep -n "CACHE_NAME\s*=" webapp/library/sw.js` to find the version constant.
3. List changed files under `webapp/library/`: if any file other than `sw.js` changed, `sw.js` must be in the diff with a bumped version.
...
1. **SW not bumped.** Files under `webapp/library/` changed, but `sw.js` not in the diff OR `CACHE_NAME` unchanged → severity 85–95, category `sw-version-not-bumped`.
