# Pendências de imagem — cards anatomia v2

Gerado automaticamente por `tools/report_pending_images.mjs`. Rode o script e commite junto com cada PR de tema.

| Tema | v2 cards | Com imagem | Pendentes |
|---|---|---|---|
| Abdominoplastia | 5 | 4 | 1 |
| Blefaroplastia | 20 | 5 | 15 |
| Contorno Pós-Bariátrico | 12 | 5 | 7 |
| Gluteoplastia | 8 | 5 | 3 |
| Lipoaspiração | 4 | 4 | 0 |
| Otoplastia | 2 | 2 | 0 |
| Rinoplastia | 15 | 5 | 10 |
| Ritidoplastia (Facelift) | 4 | 4 | 0 |
| **Total** | | | **36** |

## Abdominoplastia (contorno-corporal/abdominoplastia)

- `abdo-anat-005` — Gordura Visceral

## Blefaroplastia (estetica-facial/blefaroplastia)

- `blef-anat-001` — Pele Pálpebral
- `blef-anat-003` — Septo Órbital
- `blef-anat-004` — Gordura Orbitária — Pálpebra Superior
- `blef-anat-005` — Gordura Orbitária — Pálpebra Inferior
- `blef-anat-007` — Músculo de Müller (Tarsal Superior)
- `blef-anat-009` — Tarso (Superior e Inferior)
- `blef-anat-010` — Ligamento Cantal Medial
- `blef-anat-011` — Ligamento Cantal Lateral e Tendão Cantal Lateral
- `blef-anat-012` — Ligamento de Whitnall
- `blef-anat-013` — Vascularização Periórbital
- `blef-anat-014` — Inervação Pálpebral — Sensitiva e Motora
- `blef-anat-016` — Ligamento de Retenção Orbicular (ORL)
- `blef-anat-017` — Pálpebra Asiática — Particularidades Anatômicas
- `blef-anat-019` — ROOF (Retro-Orbicularis Oculi Fat)
- `blef-anat-020` — Retináculo Lateral

## Contorno Pós-Bariátrico (contorno-corporal/contorno-pos-bariatrico)

- `cpb-anat-002` — Monte Púbico
- `cpb-anat-007` — Parede Abdominal Pós-Bariátrica (ficha complementar)
- `cpb-anat-008` — Monte Púbico (ficha complementar)
- `cpb-anat-009` — Coxa Medial (ficha complementar)
- `cpb-anat-010` — Braço (ficha complementar)
- `cpb-anat-011` — Mama Pós-Bariátrica (ficha complementar)
- `cpb-anat-012` — Face e Pescoço (ficha complementar)

## Gluteoplastia (contorno-corporal/gluteoplastia)

- `glut-anat-001` — Artéria Glútea Superior
- `glut-anat-007` — Nervo Glúteo Superior
- `glut-anat-008` — Nervo Glúteo Inferior

## Rinoplastia (estetica-facial/rinoplastia)

- `rino-anat-003` — Pirâmide Óssea — Terço Superior
- `rino-anat-004` — Cartilagens Laterais Superiores (CLS)
- `rino-anat-005` — Área do Keystone
- `rino-anat-007` — Scroll Area
- `rino-anat-009` — Ligamentos Nasais
- `rino-anat-011` — Inervação Nasal
- `rino-anat-012` — Musculatura Nasal
- `rino-anat-013` — Cornetos (Turbinatos) Nasais
- `rino-anat-015` — Valva Nasal Externa
- `rino-anat-016` — Mecanismos de Suporte da Ponta Nasal
